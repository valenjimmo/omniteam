...
const nav = [
  { label: "Dashboard", icon: LayoutDashboard }, { label: "Attendance", icon: ClipboardCheck, active: true },
...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><Sparkles size={17} /></div><div><strong>omni<span>team</span></strong><small>OMNIATHLETE</small></div></div>
      ...
      <nav>{nav.map(({ label, icon: Icon, active }) => <a className={active ? "active" : ""} href="#" key={label}><Icon size={18} /><span>{label}</span>{label === "Attendance" && <i>2</i>}</a>)}</nav>
