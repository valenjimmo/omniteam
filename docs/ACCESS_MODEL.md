...
`/platform/login` provides the dedicated platform-owner password sign-in and `/platform` provides the platform console. The account must first be created in Supabase Auth and then granted platform-owner status through the trusted `supabase/scripts/provision_platform_owner.sql` operation. The console can create an isolated organization/team, assign the platform owner as that team's OWNER, and enable OmniSite. MFA enrollment and database-side `aal2` enforcement remain incomplete; do not use this account with real customer data until those are added.
