...
`/platform/login` provides the dedicated platform-owner password sign-in and `/platform` is server-gated before rendering its owner console. The account must first be created in Supabase Auth and then granted platform-owner status through the trusted `supabase/scripts/provision_platform_owner.sql` operation. The owner dashboard can see all teams and subscription states, directly create an OmniSite team, apply mock plans, override module entitlements, archive/reactivate teams, and reach templates, audited support, and administration API tools. Its database mutations re-check platform-owner status and write team audit records. MFA enrollment and database-side `aal2` enforcement remain incomplete; do not use this account with real customer data until those are added.
